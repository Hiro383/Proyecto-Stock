...
public class ProductoDAO implements CRUDProducto{
    ...
    private static String sql_insert = "insert into producto (codigo_producto, nombre_producto, descripcion_producto, estado_producto, "
            + " cantidad_producto, valorNeto_producto, valorIva_producto, valorTotal_producto, marca_producto, serial_producto ,"
            + " stock_minimo_producto, fecha_ingreso_producto, fecha_salida_producto, fecha_capital_producto, codigo_activo_producto,"
            + " codigo_bodega, id_tipo_producto, id_tipo_unidad) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    private static String sql_delete = "delete from producto where codigo_producto = ?";
    private static String sql_selectProducto = "select * from producto where codigo_producto = ?";
    private static String sql_update = "update producto set nombre_producto = ?, descripcion_producto = ?, estado_producto = ?,"
            + " cantidad_producto = ?, valorNeto_producto = ?, valorIva_producto = ?, valorTotal_producto = ?, marca_producto = ?, serial_producto = ?,"
            + " stock_minimo_producto = ?, fecha_ingreso_producto = ?, fecha_salida_producto = ?, fecha_capital_producto = ?, codigo_activo_producto = ?,"
            + " codigo_bodega = ?, id_tipo_producto = ?, id_tipo_unidad = ?  where codigo_producto = ?";
    ...
    public boolean agregarProducto(Producto objProducto) {
        ...
        psI.setString(1, objProducto.getCodigo_producto());
        ...
        psI.setInt(15, objProducto.getCodigo_activo_producto());
        psI.setString(16, objProducto.getCodigo_bodega());
        psI.setInt(17, objProducto.getId_tipo_producto());
        psI.setInt(18, objProducto.getId_tipo_unidad());
    ...
    try {
            ...
            psU.setInt(14, objProducto.getCodigo_activo_producto());
            psU.setString(15, objProducto.getCodigo_bodega());
            psU.setInt(16, objProducto.getId_tipo_producto());
            psU.setInt(17, objProducto.getId_tipo_unidad());
            
            psU.setString(18, objProducto.getCodigo_producto());
    ...
    public Producto eliminarProducto(Producto objProducto) {
        try {
            ...
            psD.setString(1, objProducto.getCodigo_producto());
    ...
    public ArrayList<Producto> listarProducto() {
        ...
        try{
            ...
            while(rs.next()){
                ...
                objProducto.setCodigo_producto(rs.getString("codigo_producto"));
    ...
    public Producto buscarPorId(Producto objProducto) {
        try {
            ...
            psG.setString(1, objProducto.getCodigo_producto());
    ...
    public List buscar(String texto) {
        ...
        String sql_search = "SELECT * FROM producto WHERE codigo_producto like '%"+texto+"%' or nombre_producto like '%"+texto+"%'"
                ...
                + " or fecha_ingreso_producto like '%"+texto+"%' or fecha_salida_producto like '%"+texto+"%' or fecha_capital_producto like '%"+texto+"%'"
                + " or codigo_activo_producto like '%"+texto+"%' or codigo_bodega like '%"+texto+"%' or id_tipo_producto like '%"+texto+"%' or id_tipo_unidad like '%"+texto+"%'";       
