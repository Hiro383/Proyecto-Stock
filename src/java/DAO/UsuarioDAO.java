...
public class UsuarioDAO implements CRUDUsuario {
    ...
    private static String sql_insert = "insert into usuario (rut_usuario, nombre_usuario, password_usuario,ape_paterno_usuario, ape_materno_usuario, "
            + "celular_usuario, correo_usuario, direccion_usuario, id_tipo_usuario, codigo_centro_costo) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    private static String sql_delete = "delete from usuario where rut_usuario = ?";
    private static String sql_selectProducto = "select * from usuario where rut_usuario = ?";
    private static String sql_update = "update usuario set nombre_usuario = ?, password_usuario = ?, ape_paterno_usuario = ?, ape_materno_usuario = ?, celular_usuario = ?, correo_usuario = ?, direccion_usuario = ?, id_tipo_usuario = ?, codigo_centro_costo = ?  where rut_usuario = ?";
    ...
    public boolean agregarUsuario(Usuario objUsuario) {
        ...
        psI.setString(10, objUsuario.getCodigo_centro_costo());
    ...
    public boolean editarUsuario(Usuario objUsuario) {
        try {
            ...
            psU.setString(1, objUsuario.getNombre_usuario());
            psU.setString(2, objUsuario.getPassword_usuario());
            psU.setString(3, objUsuario.getApe_paterno_usuario());
            psU.setString(4, objUsuario.getApe_materno_usuario());
            psU.setString(5, objUsuario.getCelular_usuario());
            psU.setString(6, objUsuario.getCorreo_usuario());
            psU.setString(7, objUsuario.getDireccion_usuario());
            psU.setInt(8, objUsuario.getId_tipo_usuario());
            psU.setString(9, objUsuario.getCodigo_centro_costo());
            psU.setString(10, objUsuario.getRut_usuario());
    ...
    ArrayList<Usuario> lstUsuario = new ArrayList<>();
        try{
            ...
            while(rs.next()){
                ...
                objUsuario.setCodigo_centro_costo(rs.getString("codigo_centro_costo"));
    ...
    public List buscar(String texto) {
        ...
        String sql_search = "select * from usuario where rut_usuario like '%"+texto+"%' or nombre_usuario like '%"+texto+"%'"
                ...
                + "or codigo_centro_costo like '%"+texto+"%' or password_usuario like '%"+texto+"%' ";
        ...
        try {
            ...
            while(rs.next()){
                ...
                u.setCodigo_centro_costo(rs.getString("codigo_centro_costo"));
                lista.add(u);
            }
